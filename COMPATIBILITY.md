...
| Window-reopening notification (`utp_read_drained`) | **Cited + partly measured** | `connection.onReadDrained`, with `processReads` moved to run before `flushAck` so an acknowledgement carries the window the drain produced, as libutp's does (`utp_internal.cpp:3242-3261`). Five unit cases pin the rules, including that a sub-packet window counts as growth. **What is not measured:** the end-to-end benefit. Three network measurements failed to hold still; the cause turned out to be a gap deadlock in the receive buffer (since fixed, see the row below), which also means the stall-rate figures once credited to this mechanism were that deadlock and are retracted. A clean measurement is now possible and has not been taken. |
