...
| Clock-skew correction | **None** | libutp keeps a delay history for the peer's direction (`their_hist`) and shifts its own delay base when the peer's base drops, on the grounds that shared drift moves both together (`utp_internal.cpp:2002-2014`). We keep the raw value (`PeerTsDiff`) but no history, so no correction. Untestable here at present: netem's endpoints share one clock, so no skew can be produced. See KNOWN-LIMITATIONS.md |
