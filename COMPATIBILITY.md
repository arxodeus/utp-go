...
| Area | Evidence | Notes |
| --- | --- | --- |
| Data retransmission schedule | **Measured** | 1x, 3x, 7x, 15x its 1000ms floor. Ours matches. With a window in flight it did not: we resent the whole window at a timeout, and again an undoubled interval later. Now libutp's: the oldest at the timeout, the rest oldest first under the window or one per acknowledgement through the fast-timeout retry, which was missing. `TestRetransmissionTimeoutResendsOnlyTheOldest`; the blackout measurement shows one packet at the timeout on both sides. Costs about 6% at 5% loss (classic LEDBAT, 12 seeds), still faster than libutp there: KNOWN-LIMITATIONS.md, section 2f |
| Retransmission timeout computation | **Measured** | `rto = max(rtt + rtt_var*4, 1000)`, `:1362-1380`. `TestConformanceRetransmissionTimeoutComputation` feeds both the same chains of round trips on virtual clocks and measures when each resends: libutp's timeout to 10ms (its 500ms timeout pass is swept in phase to find the deadline), ours to our 25ms wheel tick. Five cases, all within a tick. Measuring it found two defects: no RTT sample was taken from the SYN, and every acknowledgement that retired everything in flight returned before restarting the deadline. Both fixed: KNOWN-LIMITATIONS.md, section 2g |
| Fast retransmit threshold and cap | **Measured** | Three packets acknowledged past a hole, at most four resent per ack, oldest first (`selective_ack`, `:1480-1610`); a bare duplicate ack resends nothing. `TestConformanceFastRetransmitDecisions` grows both windows past 14 packets, sends each the same selective acks relative to its own oldest packet, and compares which packets each resends. Five scenarios, identical on both sides, and libutp's own answers match its source |
| `fast_resend_seq_nr` | **Measured** | Was absent; one packet was resent up to 16 extra times. Now compared: a second ack with the same holes resends the next two, not the same four again, on both sides (`TestConformanceFastRetransmitDecisions`) |
| Zero-window probe | **Measured, diverges** | Was absent, and its absence was a deadlock. `TestConformanceZeroWindowProbeTiming`: at libutp's default MTU (1402) both probe after 15s. At an Ethernet interface's MTU (1472) libutp never probes -- its probe opens the window to PACKET_SIZE, 1435, and a 1452-byte packet does not fit -- while ours still does. A recorded deviation: DEVIATIONS.md |
...
| Area | Evidence | Notes |
| --- | --- | --- |
| LEDBAT window adjustment | **Differential** | Rule by rule against libutp itself: `TestConformanceLedbatRules` drives libutp on its virtual clock, captures the line `apply_ccontrol` logs for every acknowledgement (its inputs and the window it chose, `utp_internal.cpp:1713-1730`, via the driver's `EnableCCLog`), replays those inputs into ours and compares the window to the byte. Three traces: a full window through slow start, the 0.9-target exit, increase and decrease (79 updates); a sender that never fills its window (34); and one that fills it, drains and trickles past the one-second guard (44). All match. Six deliberate breakages -- the exit threshold, the window factor, slow start ignoring the LEDBAT term, the delay halved, the guard removed, the guard at five seconds -- each fail it. It found one defect: before its window first filled, a sender was not treated as application-limited, where libutp's is (KNOWN-LIMITATIONS.md 2h). Not covered: the `ssthresh` exit (it needs a window at `opt_sndbuf`) and the drift penalty, measured separately. The aggregate delay response is also compared: `netem.TestCongestionRespondsToSelfInflictedQueue`, a standing queue of 112-113ms for libutp and 118ms for ours |
...
| Incoming-connection cap (`GetCount() > 3000`) | **Measured** | Was absent. `utp.WithMaxConnections` / `utpnet.Options.MaxConnections`, default libutp's 3000: same count, same `>` comparison, same place before the firewall, same silent refusal (`utp_internal.cpp:2967-2974`). Six unit cases, one failing per rule broken, and `utpnet.TestMaxConnectionsRefusesPastTheCap` over real UDP |
