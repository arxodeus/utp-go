...
| Window-reopening notification (`utp_read_drained`) | **Cited + partly measured** | `connection.onReadDrained`, with `processReads` moved to run before `flushAck` so an acknowledgement carries the window the drain produced, as libutp's does (`utp_internal.cpp:3242-3261`). Five unit cases pin the rules, including that a sub-packet window counts as growth. **What is not measured:** the end-to-end benefit. Three network measurements failed to hold still and the reason was not established; what is measured is the stall rate in a hostile configuration, 4-in-20 without the mechanism against about 1-in-20 with it. See KNOWN-LIMITATIONS.md, including a separate stall this scenario reaches that is still open. |
| Receive-window accounting for reordered data | **Differential — a confirmed divergence** | `receiveBuffer.Available()` charges every byte held out of order against the advertised window; libutp's `get_rcv_window` (`utp_internal.cpp:590-596`) counts only what its embedder has not yet read, and reordered packets never reach that count. Measured with a gap left open: 40,000 bytes held costs us exactly 40,000 of window and libutp none; 800 packets drive ours to 1,376 bytes — under one packet, so the sender is blocked, and above zero, so the peer's zero-window probe never arms. This is the cause of the stall recorded in KNOWN-LIMITATIONS.md. **Not fixed.** |
